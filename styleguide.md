...
- Align arguments and patterns in function definitions at the start of each one rather than inside the argument.
...
Bitwise operators: `_&_` (and), `_|_` (or), `_^_` (xor), `_~_` (not), `_<<_` (left shift), `_>>_` (right shift).
...
- Use `with` for pattern matching instead of `case_of_` or `if_then_else_`.
...
- file: Base/String/hash-rotate-left.agda

```agda
module Base.String.hash-rotate-left where
...
- Prefer tests using `_≡_` and `refl` as proofs.
