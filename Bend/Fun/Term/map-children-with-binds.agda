...
map-children-with-binds f (Mat bnd arg with-bnd with-arg arms) =
  Mat bnd (f (Maybe.to-list bnd) arg)
      with-bnd
      (map (λ { (mb , t) → f (Maybe.to-list mb) t }) (zip with-bnd with-arg))
      (map (λ rule → record rule { bod = f (concat-maybes (MatchRule.bnd rule)) (MatchRule.bod rule) }) arms)
map-children-with-binds f (Swt bnd arg with-bnd with-arg pred arms) =
  Swt bnd (f (Maybe.to-list bnd) arg)
      with-bnd
      (map (λ { (mb , t) → f (Maybe.to-list mb) t }) (zip with-bnd with-arg))
      pred
      (map (f (Maybe.to-list pred)) arms)
map-children-with-binds f (Fold bnd arg with-bnd with-arg arms) =
  Fold bnd (f (Maybe.to-list bnd) arg)
       with-bnd
       (map (λ { (mb , t) → f (Maybe.to-list mb) t }) (zip with-bnd with-arg))
       (map (λ rule → record rule { bod = f (concat-maybes (MatchRule.bnd rule)) (MatchRule.bod rule) }) arms)
map-children-with-binds f (Bend bnd arg cond step base) =
  Bend bnd (map (λ { (mb , t) → f (Maybe.to-list mb) t }) (zip bnd arg))
       (f [] cond)
       (f [] step)
       (f [] base)
