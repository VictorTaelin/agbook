...
hash : String → Bits
hash str = do
  let words = (map to-nat (to-list str))
  ...
  rotate-left n shift width = do
    let lower = div n (2 exp (width - shift))
    let upper = (n * (2 exp shift)) % (2 exp width)
    (upper + lower)
  ...
  fxhash-step char hash = do
    let seed = 0x517cc1b727220a95
    let hash = rotate-left hash 5 64
    let hash = hash xor char
    let hash = hash * seed
    let hash = hash % (2 exp 64)
    hash
