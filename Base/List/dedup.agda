...
dedup-go : ∀ {A : Set} {{_ : Ord A}} → List A → OrdSet A → List A
dedup-go [] acc = []
dedup-go (x :: xs) acc with contains x acc
... | True  = dedup-go xs acc
... | False = x :: dedup-go xs (x ::> acc)
