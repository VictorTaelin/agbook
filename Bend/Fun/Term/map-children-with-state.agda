module Bend.Fun.Term.map-children-with-state where
...
map-children-with-state : ∀ {A : Set} → (A → Term → Pair A Term) → A → Term → Pair A Term
map-children-with-state f state (Lam pat bod) = do
  let (state , bod) = f state bod
  (state , Lam pat bod)
map-children-with-state f state (Var nam) = (state , Var nam)
map-children-with-state f state (Link nam) = (state , Link nam)
map-children-with-state f state (Let pat val nxt) = do
  let (state , val) = f state val
  let (state , nxt) = f state nxt
  (state , Let pat val nxt)
map-children-with-state f state (With typ bod) = do
  let (state , bod) = f state bod
  (state , With typ bod)
map-children-with-state f state (Ask pat val nxt) = do
  let (state , val) = f state val
  let (state , nxt) = f state nxt
  (state , Ask pat val nxt)
map-children-with-state f state (Use nam val nxt) = do
  let (state , val) = f state val
  let (state , nxt) = f state nxt
  (state , Use nam val nxt)
map-children-with-state f state (App fun arg) = do
  let (state , fun) = f state fun
  let (state , arg) = f state arg
  (state , App fun arg)
map-children-with-state f state (Fan kind els) = do
  let (state , els) = map-list f state els
  (state , Fan kind els)
map-children-with-state f state (Num val) = (state , Num val)
map-children-with-state f state (Str val) = (state , Str val)
map-children-with-state f state (List els) = do
  let (state , els) = map-list f state els
  (state , List els)
map-children-with-state f state (Oper op fst snd) = do
  let (state , fst) = f state fst
  let (state , snd) = f state snd
  (state , Oper op fst snd)
map-children-with-state f state (Mat bnd arg with-bnd with-arg arms) = do
  let (state , arg)      = f state arg
  let (state , with-arg) = map-list f state with-arg
  let (state , arms)     = map-arms f state arms
  (state , Mat bnd arg with-bnd with-arg arms)
map-children-with-state f state (Swt bnd arg with-bnd with-arg pred arms) = do
  let (state , arg)      = f state arg
  let (state , with-arg) = map-list f state with-arg
  let (state , arms)     = map-list f state arms
  (state , Swt bnd arg with-bnd with-arg pred arms)
map-children-with-state f state (Fold bnd arg with-bnd with-arg arms) = do
  let (state , arg)      = f state arg
  let (state , with-arg) = map-list f state with-arg
  let (state , arms)     = map-arms f state arms
  (state , Fold bnd arg with-bnd with-arg arms)
map-children-with-state f state (Bend bnd arg cond step base) = do
...
map-children-with-state f state (Open typ var bod) = do
  let (state , bod) = f state bod
  (state , Open typ var bod)
map-children-with-state f state (Ref nam) = do
  (state , Ref nam)
map-children-with-state f state (Def def nxt) = do
  let (state , nxt) = f state nxt
  (state , Def def nxt)
map-children-with-state f state Era = do
  (state , Era)
